...
from inbox.util.cache import rm_cache
...
class GmailFolderSyncEngine(CondstoreFolderSyncEngine):
    ...
    def initial_sync_impl(self, crispin_client):
        ...
        try:
            ...
            remote_g_metadata = self.__fetch_g_metadata(remote_uids)
            ...
            if self.folder_name in uid_download_folders(crispin_client):
                ...
                if self.is_inbox(crispin_client):
                    with self.conn_pool.get() as all_mail_crispin_client:
                        all_mail_crispin_client.select_folder(
                            crispin_client.folder_names()['all'],
                            uidvalidity_cb)
                        self.__download_queued_threads(all_mail_crispin_client,
                                                       download_stack)
                else:
                    # We're in the All Mail folder, can use the crispin client
                    # we already have.
    ...
    def __fetch_g_metadata(self, crispin_client, uids):
        ...
        return crispin_client.g_metadata(uids)
            ...
            with self.syncmanager_lock:
                ...
                with mailsync_session_scope() as db_session:
                    ...
                    for msg in msgs_to_process:
                        add_new_imapuid(db_session, msg, self.folder_name, acc)
